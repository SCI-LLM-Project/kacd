...
causal_literature_drop = ['Plausibility', 'Plausibility Reasoning', 'Association', 'Association Reasoning', 'context', 'Temporality', 'Temporality Reasoning']
...
kg_llm_causal_lit = kg_llm_df.drop(columns=causal_literature_drop)
...
llm_causal_lit = llm_df.drop(columns=causal_literature_drop)
...
rag_causal_lit = rag_df.drop(columns=causal_literature_drop)
...
def resolve_bidirectional_edges(df, metric_col, metric_reasoning_col, metric_report_col, report_type, generator, prompt_func):
    ...
    fwd_true_rev_false = merged[
        ...
        ((merged[f'{metric_col}_rev'] == False) & (merged['Proto_rev'] == False))
    ]

    # this is probably redundant, but whatever
    fwd_false_rev_true = merged[
        ((merged[f'{metric_col}_fwd'] == False) & (merged['Proto_fwd'] == False)) &
        ((merged[f'{metric_col}_rev'] == True) | (merged['Proto_rev'] == True))
    ]
    ...
    fwd_cols = ['Var1_fwd', 'Var2_fwd', f'{metric_col}_fwd', f'{metric_reasoning_col}_fwd', f'{metric_report_col}_fwd', 'Proto_fwd']
    rev_cols = ['Var1_rev', 'Var2_rev', f'{metric_col}_rev', f'{metric_reasoning_col}_rev', f'{metric_report_col}_rev', 'Proto_rev']
    ...
    sex_edges = df[(df["Var1"] == "Sex") & ((df[metric_col] == True) | (df["Proto"] == True))]
    age_edges = df[(df["Var1"] == "Age") & ((df[metric_col] == True) | (df["Proto"] == True))]
    peg_edges = df[(df["Var2"] == "PEG") & ((df[metric_col] == True) | (df["Proto"] == True))]
...
kg_llm_causal_lit_resolved = resolve_bidirectional_edges(
    kg_llm_causal_lit, 'Causal Literature Prediction', 'Causal Literature Prediction Reasoning', 'Causal Literature Report', 'KG-RAG', generator, create_causal_prompt_kg
)

# Save KG+LLM context
kg_llm_plausibility_resolved.to_csv(f"{output_dir}/kg_llm_plausibility_with_proto_resolved.csv", index=False)
kg_llm_association_resolved.to_csv(f"{output_dir}/kg_llm_association_with_proto_resolved.csv", index=False)
kg_llm_temporality_resolved.to_csv(f"{output_dir}/kg_llm_temporality_with_proto_resolved.csv", index=False)
kg_llm_causal_lit_resolved.to_csv(f"{output_dir}/kg_llm_causal_lit_with_proto_resolved.csv", index=False)
...
llm_causal_lit_resolved = resolve_bidirectional_edges(
    llm_causal_lit, 'Causal Literature Prediction', 'Causal Literature Prediction Reasoning', 'Causal Literature Report', 'LLM', generator, create_causal_prompt_llm
)

# Save LLM context
llm_plausibility_resolved.to_csv(f"{output_dir}/llm_plausibility_with_proto_resolved.csv", index=False)
llm_association_resolved.to_csv(f"{output_dir}/llm_association_with_proto_resolved.csv", index=False)
llm_temporality_resolved.to_csv(f"{output_dir}/llm_temporality_with_proto_resolved.csv", index=False)
llm_causal_lit_resolved.to_csv(f"{output_dir}/llm_causal_lit_with_proto_resolved.csv", index=False)
...
rag_causal_lit_resolved = resolve_bidirectional_edges(
    rag_causal_lit, 'Causal Literature Prediction', 'Causal Literature Prediction Reasoning', 'Causal Literature Report','RAG', generator, create_causal_prompt_rag
)

print("\nAll resolutions complete!")

# Save RAG context
rag_plausibility_resolved.to_csv(f"{output_dir}/rag_plausibility_with_proto_resolved.csv", index=False)
rag_association_resolved.to_csv(f"{output_dir}/rag_association_with_proto_resolved.csv", index=False)
rag_temporality_resolved.to_csv(f"{output_dir}/rag_temporality_with_proto_resolved.csv", index=False)
rag_causal_lit_resolved.to_csv(f"{output_dir}/rag_causal_lit_with_proto_resolved.csv", index=False)
